...
* **결론:** 두 칩의 물리적 처리 능력(Peak MACs/Cycle)이 131,072로 동일하게 주어지더라도, TPU는 정적 스케줄링 제약으로 인해 실제 소요 시간이 459 Cycle 이상으로 늘어지는 반면, Hopper는 **이론적 하한선인 82 Cycle에 가까운 성능(82 + α)**을 대부분 방어해낸다. 이것이 MoE 추론 환경에서 Hopper의 아키텍처가 더 유리한 이유이다.

### 3.2. TPU가 유리한 시나리오: 대규모 정규 GEMM (예: LLM 대규모 Prompt Prefill)

수천~수만 개의 토큰을 한 번에 처리하는 LLM의 Prompt Prefill(입력 처리) 상황을 가정한다. 이 시기에는 토큰들이 쪼개지지 않고 거대한 단일 행렬로 한꺼번에 입력된다. 
`Batch Size × Sequence Length` = 8,192이고, `Hidden Dimension`이 4,096인 거대한 단일 GEMM 연산이라고 가정해 보자.
...
* **TPU에게 유리한 점 (Systolic Array의 고연산 밀도):**
    * **Zero-padding 부재:** 연산 크기(8,192, 4,096 등)가 TPU의 128×128 Array 크기에 완벽하게 나누어떨어지므로 빈 공간이 발생하지 않는다. 따라서 Array Utilization이 100%에 근접한다.
    * **Overhead 상쇄 (Weight Double Buffering 전제):** 각 PE가 Weight Register를 2개 보유하므로, 현재 타일 연산 중에 다음 타일의 Weight를 백그라운드로 미리 로드할 수 있다. 따라서 N/K 차원의 타일링이 발생하더라도 타일 간 전환 시 파이프라인이 멈추지 않으며, Fill/Drain Overhead는 최초 Fill(127 Cycle)과 최종 Drain(127 Cycle)의 합산인 254 Cycle 1회만 발생한다. 이는 전체 실행 시간(1,048,576 Cycle) 대비 **0.02% 수준**으로 무시할 수 있는 수치이다.
    * **명령어 제어 로직 최소화:** 단일 명령어(예: MatrixMultiply) 하나로 수십~수백만 번의 MAC 연산을 연속적으로 구동할 수 있다. GPU처럼 수많은 개별 코어가 매 사이클마다 명령어를 Fetch/Decode할 필요가 없으므로, 제어 로직에 소모되는 전력이 최소화되어 전성비(TOPS/Watt) 효율이 높다.
* **Hopper에게 불리한 점 (Control Logic Overhead):**
    * **동적 스케줄링 오버헤드 누적:** 연산이 단순하고 거대하더라도, GPU 구조상 32개의 SM은 매 사이클 개별적으로 명령어를 Fetch/Decode 하고, 수천 개의 Warp Context를 Switching하며, 레지스터와 SRAM(Shared Memory) 간의 데이터 이동을 지속적으로 수행해야 한다.
    * **결론:** Hopper 역시 연산 속도 자체는 이상적인 수치(1,048,576 Cycle)에 근접하게 처리해 내겠지만, 그 과정에서 Control Logic Overhead로 인한 전력 소모가 발생한다. 즉, 대규모 정규 연산(Prefill) 환경에서는 동적 스케줄링의 유연함(Flexibility)을 배제하고 연산 밀도(Dense Math)에 트랜지스터를 집중시킨 TPU의 아키텍처가 전성비(TOPS/Watt) 측면에서 높은 효율을 달성한다.
