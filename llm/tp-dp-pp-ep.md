...
대규모 언어 모델(LLM)의 분산 학습 및 추론에는 세 가지 핵심 병렬 처리 전략이 널리 활용됩니다: **Tensor Parallelism (TP)**, **Pipeline Parallelism (PP)**, **Data Parallelism (DP)** [[참고자료](https://rocm.blogs.amd.com/software-tools-optimization/vllm-moe-guide/README.html#references)].

> **참고**: Expert Parallelism (EP)은 MoE(Mixture of Experts) 모델을 위한 병렬화 방식으로, TP 또는 DP와 결합하여 작동합니다.

---

## 1. Tensor Parallelism (TP)

- **원리 (What it does)**: 개별 레이어를 여러 GPU에 Sharding(분할)합니다. 각 GPU는 각 레이어의 일부를 처리하며, 집합 통신(collective communication)을 통해 결과를 동기화합니다.

![Figure 1: Data flow for Tensor Parallelism when Tensor parallel=4](./images/tp_vanilla_improved.svg)  
...
- **제약 사항 (Constraint)**:
  - Attention heads 수의 제약을 받으며, 병렬 크기로 나누어 떨어져야 합니다.
...
![Figure 2: Data flow for Data Parallelism for DP=4](./images/data_parallelism.svg)  
...
![Figure 3: Data flow for Vanilla Pipeline Parallelism for PP=4](./images/vanilla_pipeline_parallelism.svg)  
*Figure 3: Data flow for Vanilla Pipeline Parallelism for PP=4*

- **Figure 3 설명 (Vanilla PP 기준)**:
  - 개념 설명을 위한 Vanilla Pipeline Parallelism에서는 하나의 요청이 순차적으로 처리됩니다.
...
- **파이프라인 스케줄링 및 인터리빙 최적화 (Pipeline Scheduling & Interleaving)**:
  - 실제 LLM 시스템(학습 및 추론 프레임워크)에서는 파이프라인 버블을 최소화하기 위해 **마이크로배치(Micro-batch) 또는 여러 요청을 동시에 파이프라인에 주입**합니다.
  - GPU 0이 요청 B를 처리하는 동안 GPU 1은 요청 A를 처리하고, GPU 2는 그보다 앞선 요청을 처리하는 방식으로 오버랩(Overlap)합니다.
...
- **핵심 통찰 (The Key Insight)**: MoE 모델의 병목은 연산량(Compute)이 아니라 **메모리 대역폭(Memory Bandwidth)**입니다.
  - MoE 모델은 Sparse Activation 특성을 가집니다 (예: DeepSeek은 토큰당 671B 파라미터 중 37B만 사용).
  - GPU는 계산보다 메모리에서 Expert weights를 로드하는 데 대부분의 시간을 소비합니다.
  - 단일 GPU의 메모리 대역폭 한계로 인해 Expert에 접근할 수 있는 속도가 제한됩니다.
- **해결책 (Solution)**: Expert들을 여러 GPU에 분산 배치하여 총 집합 메모리 대역폭(Aggregate Memory Bandwidth)을 확보합니다.
  - 8대의 GPU에 EP를 활성화하면 Expert weights 로딩에 대해 **8배의 메모리 대역폭**을 확보할 수 있습니다.
  - 이를 통해 MoE 모델의 주된 지연 요인인 메모리 전송 대기 시간을 줄입니다.
- **통신 트레이드오프 (Communication Tradeoff)**:
  - EP는 토큰을 서로 다른 GPU에 있는 Expert에게 라우팅하기 위해 `AllToAll` 통신이 필요합니다.
...
아래 Figure 4와 같이 TP=8 및 Expert Parallelism을 활성화하면 Expert들이 8개의 GPU에 분할(GPU당 32개 Expert)되며, 각 레이어 이후 `AllReduce` 통신이 사용됩니다.

![Figure 4: Data flow for TP=8 on a single node (with Expert Parallelism)](./images/tp_moe_EP_v2.svg)  
*Figure 4: Data flow for TP=8 on a single node (with Expert Parallelism)*

- **사용 시점 (When to use TP+EP)**:
  - 단일 GPU에 들어가지 않는 대형 MoE 모델
  - 지연 시간(Latency)이 가장 중요한 저~중간 수준의 Concurrency 워크로드
...
- 서로 다른 GPU가 서로 다른 요청을 처리합니다 (Request-level Parallelism).
- **KV 캐시가 GPU 간에 분할(Partitioned)**됩니다: 각 GPU는 할당된 요청에 대한 캐시만 보유합니다.
- Non-MoE 레이어는 복제(Replicated)되지만 KV 캐시는 분할된 상태를 유지합니다.
- MoE Expert는 모든 GPU에 걸쳐 분산 배치됩니다.
- 레이어 간 통신은 `AllToAll`을 통해 이루어집니다.

이 아키텍처는 아래 Figure 7에 나와 있으며, Expert들이 여러 GPU에 분산되고 `AllToAll` 통신을 통해 분할된 KV 캐시로 효율적인 요청 수준 병렬 처리를 가능하게 합니다.

![Figure 7: Data flow for DP=8 with Expert Parallelism](./images/dp_with_ep_moe_v2.svg)  
